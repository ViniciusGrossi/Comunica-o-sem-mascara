      ...
      <div className="relative mx-auto max-w-[var(--container-max)] px-6">
        <RevealWrapper direction="up">
          <div className="text-center">
            <span className="eyebrow text-gold/80">Este lugar é seu</span>
            <h2 className="mt-4 text-white">Para quem é o programa</h2>
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {TARGET_AUDIENCE.map((item, i) => (
            <RevealWrapper key={item.label} delay={i * 80} direction="up">
              <TiltCard
                max={6}
                className="group h-full cursor-default border border-gold/25 bg-white/[0.04] shadow-sm transition-shadow duration-500 hover:shadow-xl hover:shadow-gold/20"
              >
                <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-4 p-8 text-center">
                  <div className="flex h-14 w-14 items-center justify-center rounded-full border border-gold/40 bg-gold/10 transition-all duration-500 group-hover:scale-110 group-hover:bg-gold group-hover:shadow-lg group-hover:shadow-gold/30">
                    <item.icon className="h-6 w-6 text-gold transition-colors duration-500 group-hover:text-white" />
                  </div>
